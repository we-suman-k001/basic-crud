...
let model_namespace = 'VaahCms\\Modules\\Basic\\Models\\Blogs';


let base_url = document.getElementsByTagName('base')[0].getAttribute("href");
let ajax_url = base_url + "/basic/blogs";
...
export const useBlogsStore = defineStore({
    ...
    actions: {
        ...
        watchItem(title)
        {
            if(title && title !== "")
            {
                this.item.title = vaah().capitalising(title);
                this.item.slug = vaah().strToSlug(title);
            }
        },
        //---------------------------------------------------------------------
        async getAssets() {
